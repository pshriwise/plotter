...
class MainWindow(QMainWindow):
    ...
    def _on_plot_finished(self, view_snapshot, view_params, geom_data, property_data, elapsed):
        if view_params != self.plot_manager.latest_view_params:
            return
        self.model.makePlot(view_snapshot, geom_data, property_data)
        self.resetModels()
        self.showCurrentView()
        backend = 'slice_data_raytrace' if view_params.get('use_raytraced_plots', True) else 'slice_data'
        self.statusBar().showMessage(
            f"Plot data generated by openmc.lib.{backend} in {elapsed * 1000.0:.1f} ms",
            5000,
        )
