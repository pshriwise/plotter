...
from openmc_plotter.main_window import MainWindow
...
def test_on_plot_finished_reports_openmc_call_duration():
    messages = []
    status_bar = SimpleNamespace(
        showMessage=lambda message, timeout=0: messages.append((message, timeout))
    )
    window = SimpleNamespace(
        plot_manager=SimpleNamespace(latest_view_params={'use_raytraced_plots': True}),
        model=SimpleNamespace(makePlot=lambda *args: messages.append(('makePlot', args))),
        resetModels=lambda: messages.append(('resetModels', None)),
        showCurrentView=lambda: messages.append(('showCurrentView', None)),
        statusBar=lambda: status_bar,
    )

    MainWindow._on_plot_finished(
        window,
        view_snapshot='view',
        view_params={'use_raytraced_plots': True},
        geom_data='geom',
        property_data='props',
        elapsed=0.01234,
    )

    assert ('makePlot', ('view', 'geom', 'props')) in messages
    assert ('resetModels', None) in messages
    assert ('showCurrentView', None) in messages
    assert messages[-1] == (
        'Plot data generated by openmc.lib.slice_data_raytrace in 12.3 ms',
        5000,
    )
